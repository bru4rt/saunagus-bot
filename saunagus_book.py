...
async def fill_and_submit(page):
    ...
    await page.get_by_text("Bekræft booking", exact=False).wait_for(timeout=8000)
    confirm_section = page.locator("#confirm")
    await confirm_section.wait_for(timeout=8000)

    # Fill "Navn"
    full_name = " ".join(filter(None, [FIRST_NAME.strip(), LAST_NAME.strip()])).strip()
    if full_name:
        try:
            await confirm_section.locator("input#name").fill(full_name)
        except Exception as e:
            log(f"⚠ Could not fill 'Navn': {e}")

    # Fill "E-mail"
    if EMAIL:
        try:
            await confirm_section.locator("input#email").fill(EMAIL)
        except Exception as e:
            log(f"⚠ Could not fill 'E-mail': {e}")

    # Fill "Mobil" – use input[type=tel] because of the country selector
    if MOBILE:
        try:
            phone_input = confirm_section.locator("input#mobile")
            await phone_input.click()
            await phone_input.fill(MOBILE)
        except Exception as e:
            log(f"⚠ Could not fill 'Mobil': {e}")

    # Fill "Kommentar"
    if COMMENT:
        try:
            await confirm_section.locator("textarea#comment").fill(COMMENT)
        except Exception as e:
            log(f"⚠ Could not fill 'Kommentar': {e}")

    await page.wait_for_timeout(500)

    if DRY_RUN:
        log("DRY_RUN is true – not clicking 'Bekræft'.")
        return False

    # Click the Bekræft button for real booking
    log("Clicking 'Bekræft' to finalize booking...")
    await page.get_by_role("button", name=re.compile(r"Bekræft", re.I)).click(timeout=5000)
    log("Submitted Bekræft – booking should now be in progress.")
...
async def select_guest_count(page):
    ...
    question = page.get_by_text("Hvor mange bliver I?", exact=False)
    await question.wait_for(timeout=8000)
    ...
    try:
        ...
        log("Guest count not exposed as role=button; falling back to relative text.")

    # 2) Fallback: click the first element with text '1' after the question
    # NOTE: we now bake the label directly into the XPath string (no 'label=' kwarg).
    xpath = f"xpath=following::*[normalize-space()='{guest_label}'][1]"
    one_candidate = question.locator(xpath)

    await one_candidate.click(timeout=3000)
    log(f"Selected guest count {guest_label} via text near question.")
    await page.wait_for_timeout(800)   # let calendar render
...
async def main():
    ...
    while True:
        try:
            ...
            if submitted:
                mark_booking_today()
                got = wait_for_confirmation_email()
                if got:
                    log("✅ Booking confirmed by email. Exit 0.")
                    sys.exit(0)
                else:
                    log("⚠️ No email detected. Exit 2.")
                    sys.exit(2)
            else:
