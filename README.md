...
2. **Configure `.env`**
   - Copy `.env.example` → `.env` and fill in your real details (keep `.env` out of git).
   ...
   - Set `WAIT_FOR_MIDNIGHT=false` when demoing during the day—the bot will skip the midnight sleep and start immediately.
